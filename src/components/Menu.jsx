...
const deletePizza = (pizzaName) => setPizzas(prev=>prev.filter((pizza)=>pizza.name!==pizzaName));
    return(
        <main className="menu">
            <h2>Our menu</h2>
            {pizzas.length > 0 ?
            //! Esta etiqueta se conoce como fragment y no es renderizada en la aplicación <>. Nos sirve como contenedor padre sin ser renderizado en el HTML https://react.dev/reference/react/Fragment
            (<>
                <p>
                    Authentic Italian cousine. 6 creative dishes to choose from. All from our stone oven, all organic, all delicious.
                </p>
                <ul className="pizzas">
                    {pizzas.map((pizza)=>{
                        console.log(pizza);
